      ...
      body: Stack(
        ...
        children: [
          ListView(
            ...
            children: [
                  ...
                  children: [
                    Obx(() => SizedBox(
                        ...
                        child: PageView.builder(
                            ...
                            itemCount: homeVM.dataModel.value.banners != null
                                ? homeVM.dataModel.value.banners!.length
                                : 3,
                            scrollDirection: Axis.horizontal,
                            itemBuilder: ((context, index) {
                              return homeVM.dataModel.value.banners != null
                                  ? carouselImageTile(
                                      imageUrl: homeVM.dataModel.value
                                          .banners![index].photoUrl)
                    ...
                    Padding(
                      padding: EdgeInsets.only(
                          ...
                          bottom: SizeConfig.heightMultiplier * 10.0),
                      child: Obx(
                        () => Column(
                          ...
                          children: [
                            ...
                            homeVM.dataModel.value.banners != null
                                ? carouselDots(
                                    length:
                                        homeVM.dataModel.value.banners!.length,
                                    selectedIndex:
                                        homeVM.carouselTileSelectedIndex.value)
                                : Shimmer.fromColors(
                                    child: carouselDots(),
                                    baseColor: AppColor.textGrey,
                                    highlightColor: AppColor.greyWhite,
                                  )
                          ],
                        ),
                      ),
                        ...
                        child: homeSearchField(
                            searchCallback: () {
                              if (homeVM.searchController.text != '') {
                                homeVM.searchList
                                    .add(homeVM.searchController.text);
                              }
              ...
              Padding(
                ...
                child: titleTile(title: 'Categories'),
              ),
              SizedBox(
              ...
              Obx(
                () => SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    scrollDirection: Axis.horizontal,
                    itemCount: homeVM.dataModel.value.categories != null
                        ? homeVM.dataModel.value.categories!.length
                        : 5,
                    itemBuilder: (context, index) {
                      var data = homeVM.dataModel.value.categories;
                      var featuredData =
                          homeVM.dataModel.value.featuredServices;
                      return homeVM.dataModel.value.categories != null
                          ? categoryTile(
                              index: index,
                              length: homeVM.dataModel.value.categories!.length,
                              categoriesCallBack: () {
                               
                                List<FeaturedService> dataList = featuredData!.where((element) => element.category.id == data[index].id).toList();
                          ...
                          : shimmer(Widget: shimmercategoryTile());
                    },
                  ),
                ),
              ),
                ...
                child: Obx(
                  () => ListView.builder(
                      padding: EdgeInsets.only(
                          left: SizeConfig.widthMultiplier * 5.0),
                      scrollDirection: Axis.horizontal,
                      itemCount: homeVM.dataModel.value.featuredServices != null
                          ? homeVM.dataModel.value.featuredServices!.length
                          : 3,
                      itemBuilder: (context, index) {
                          ...
                          for (int i = 0; i < data[index].slots.length; i++) {
                            return Obx(
                              () => interestTile(
                                  ...
                                  ratingTitle:
                                      data[index].totalRating.toString(),
                                  context: context,
                                  initialRating: data[index].rating.toDouble(),
                                  ratingCallback: (rating) {},
                                  index: index,
                                  length: homeVM
                                      .dataModel.value.featuredServices!.length,
                                  callBack: () {},
                                  isFavorite: data[index].isLiked,
                                  isFavoriteCallback: () {
                                    data[index].isLiked = !data[index].isLiked;
                                    print(data[index].isLiked);
                                    HomeServices().putLike(
                                        id: data[index].id,
                                        like: data[index].isLiked);
                                  },
                                  discount: '30',
                                  title: data[index].title,
                                  subtitle: data[index].location.title,
                                  description: data[index].description,
                                  location: data[index].location.about,
                    ...
                    itemBuilder: (context, index) {
                      return Obx(
                        () => homeServicesTile(
                            ...
                            isFavoriteCallback: () {
                              homeVM.comboPackageList[index].favorite.value =
                                  !homeVM
                                      .comboPackageList[index].favorite.value;
              ...
              Padding(
                padding: EdgeInsets.symmetric(
                    horizontal: SizeConfig.widthMultiplier * 5.0),
                child: seeAllTitleTile(
                    seeAllCallBack: () {}, title: 'Featured Services'),
              ),
              SizedBox(
                height: SizeConfig.heightMultiplier * 2.0,
              ),
                        ...
                        if (homeVM.dataModel.value.featuredServices != null) {
                          for (int i = 0; i < data![index].slots.length; i++) {
                            return homeServicesTile(
                                initialRating:
                                    data[index].ratingCount.toDouble(),
                                ratingCallback: (value) {},
                                isFavoriteCallback: () {
                                  data[index].isLiked = !data[index].isLiked;
                ...
                child: Obx(
                  () => ListView.builder(
                      padding: EdgeInsets.only(
                          left: SizeConfig.widthMultiplier * 5.0),
                      scrollDirection: Axis.horizontal,
                      itemCount: homeVM.dataModel.value.nearbyServices != null
                          ? homeVM.dataModel.value.nearbyServices!.length
                          : 3,
                      itemBuilder: (context, index) {
                        var data = homeVM.dataModel.value.nearbyServices;
                        if (homeVM.dataModel.value.nearbyServices != null) {
                          for (int i = 0;
                              i < homeVM.dataModel.value.nearbyServices!.length;
                              i++) {
                            return homeServicesTile(
                                initialRating: 3.0,
                                ratingCallback: (value) {},
                                isFavoriteCallback: () {
                                  ...
                                  // }
                                  print(homeVM
                                      .dataModel.value.nearbyServices!.length);
                                },
                                isFavorite: data![index].isLiked,
                                discount: '30',
                                title: data[index].title,
          ...
          Positioned(
            ...
            child: Obx(() => homeVM.isShowAderlineMeter.value == true
                ? adrenalineMeterDailog()
                : SizedBox.shrink()),
          )
        ],
      ),
      floatingActionButton: Padding(
        ...
        child: AnimatedBuilder(
          ...
          child: GestureDetector(
              onTap: () {
                homeVM.isShowAderlineMeter.value =
                    !homeVM.isShowAderlineMeter.value;
              ...
              child: Image(
                image: AssetImage('assets/home/icons/float.png'),
                fit: BoxFit.cover,
